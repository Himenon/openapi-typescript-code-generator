...
): Intermediate.PropertySignatureParams[] => {
  if (!schema.properties) {
    return [];
  }
  const required: string[] = schema.required || [];
  return Object.entries(schema.properties).map<Intermediate.PropertySignatureParams>(([propertyName, property]) => {
    if (!property) {
      return {
    ...
    return {
      ...
      schemaType: ToTypeNode.convert(entryPoint, currentPoint, factory, property, context, convertContext, { parent: schema }),
    }
  });
};
